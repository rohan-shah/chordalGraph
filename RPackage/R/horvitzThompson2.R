horvitzThompson2 <- function(nVertices, budget, seed, nEdges, options = list(reduceChains = FALSE, graphRepresentation = "list"))
{
	...
	{
		start <- Sys.time()
		result <- .Call("horvitzThompson2", nVertices, budget, seed, options, PACKAGE="chordalGraph")
		end <- Sys.time()
	...
	else
	{
		...
		start <- Sys.time()
		result <- .Call("horvitzThompsonSpecificEdges2", nVertices, nEdges, budget, seed, options, PACKAGE="chordalGraph")
		end <- Sys.time()

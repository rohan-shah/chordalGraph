...
BEGIN_RCPP
	...
	for (int nEdges = 0; nEdges < maxEdges+1; nEdges++)
	{
		...
		else
		{
			if(representation == listRepresentation)
			{
				chordalGraph::horvitzThompson2<chordalGraph::cliqueTree>(args);
			}
			else
			{
				chordalGraph::horvitzThompson2<chordalGraph::cliqueTreeAdjacencyMatrix>(args);
			}
		}
...
SEXP horvitzThompson2SpecificEdges(SEXP nVertices_sexp, SEXP nEdges_sexp, SEXP budget_sexp, SEXP seed_sexp, SEXP options_sexp)
{
BEGIN_RCPP
	int seed = Rcpp::as<int>(seed_sexp);
	int nVertices = Rcpp::as<int>(nVertices_sexp);

...
SEXP armstrongPosteriorInference(SEXP outerProductsSum_sexp, SEXP delta_sexp, SEXP dimension_sexp, SEXP dataPoints_sexp, SEXP psi_sexp, SEXP exactCounts_sexp, SEXP burnIn_sexp, SEXP runSize_sexp, SEXP seed_sexp, SEXP uniqueGraphsLimit_sexp)
{
BEGIN_RCPP
	...
	for(chordalGraph::armstrongPosteriorInferenceArgs::resultsType::iterator i = args.results.begin(); i != args.results.end(); i++)
	{
		Rcpp::IntegerMatrix currentGraphR(args.dimension, args.dimension);
		std::fill(currentGraphR.begin(), currentGraphR.end(), 0);
		const chordalGraph::armstrongPosteriorInferenceArgs::graphType& currentGraph = i->first;
		chordalGraph::armstrongPosteriorInferenceArgs::graphType::edge_iterator current, end;

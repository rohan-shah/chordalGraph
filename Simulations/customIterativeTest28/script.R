...
exactCountsEnd <- mpfr(c(choose(maxEdges, maxEdges-2) - choose(vertices, 4)*3, maxEdges, 1), precBits = 64)
...
for(edgeCounter in start:maxEdges)
{
	...
	{
		estimatedCounts[(maxEdges-1):(maxEdges+1)] <- exactCountsEnd
	}
	...
	save(customSymmetricCounts, customSymmetricResults, file = filename)
}

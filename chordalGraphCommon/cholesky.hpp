  ...
  for (size_t k=0 ; k < n; k++) {
    ...
    if (qL_kk <= 0) {
      ...
      for (size_t i = k+1; i < A.size1(); ++i) {
        ...
        if (Aik != 0) {
          *Aik = ( *Aik - ublas::inner_prod( ublas::project( ublas::row( A_c, k ), ublas::range(0, k) ), ublas::project( ublas::row( A_c, i ), ublas::range(0, k) ) ) ) / L_kk;
        }
      }
...
//   ::inplace_solve(L, x, lower_tag(), typename TRIA::orientation_category () );
  ublas::inplace_solve(L, x, ublas::lower_tag() );
  ublas::inplace_solve(trans(L), x, ublas::upper_tag());
}
